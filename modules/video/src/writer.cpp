...
bool Writer::Open(std::string REF_IN video_file, cv::Size frame_size) {
  ...
  if (!writer_->open(video_file, cv::CAP_FFMPEG, cv::VideoWriter::fourcc('m', 'p', '4', 'v'), FRAME_RATE,
                     std::move(frame_size))) {
...
bool Writer::Write(cv::Mat FWD_IN frame, double time) {
  if (writer_ && time >= time_ + 1.0 / FRAME_RATE - 1e-6) {
...
void Writer::WritingThreadFunction(void *obj) {
  ...
  while (!self->stop_flag_) {
    while (self->buffer_.Pop(frame)) self->writer_->write(frame);
