...
HikCamera::~HikCamera() {
  for (auto &&callback : callback_list_) UnregisterFrameCallback(callback.first);
...
bool HikCamera::OpenCamera(std::string REF_IN serial_number, std::string REF_IN config_file) {
  ...
  if (!daemon_thread_id_) {
    ...
    DLOG(INFO) << serial_number_ << "'s daemon thread " << daemon_thread_id_ << " started.";
...
bool HikCamera::CloseCamera() {
  ...
  stop_flag_ = false;
#if defined(__APPLE__)
  DLOG(INFO) << serial_number_ << "'s daemon thread " << daemon_thread_id_ << " stopped.";
#elif defined(__linux__)
...
void HikCamera::ImageCallbackEx(unsigned char *image_data, MV_FRAME_OUT_INFO_EX *frame_info, void *obj) {
  ...
  for (auto p : self->callback_list_) (*p.first)(p.second, frame);
  if (frame.valid) self->buffer_.Push(std::move(frame));
}
...
void *HikCamera::DaemonThreadFunction(void *obj) {
  ...
  while (!self->stop_flag_) {
    ...
    if (!MV_CC_IsDeviceConnected(self->device_)) {
      ...
      while (!self->OpenCamera(self->serial_number_, "../cache/" + self->serial_number_ + ".txt")) sleep(1);
      LOG(INFO) << self->serial_number_ << " is successfully reconnected.";
      if (self->stream_running_) self->StartStream();
    }
  }
  return nullptr;
}
